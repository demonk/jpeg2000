...
#include <iostream>
using namespace std;
int bmptoimage(char *filename,Image *img,int subsampling_dx,int subsampling_dy);
...
int main(int argc, char **argv)
{
	...
	cp.tw=jpegMath::int_ceildiv(img.Xsiz-cp.XTOsiz,cp.XTsiz);
	cp.th=jpegMath::int_ceildiv(img.Ysiz-cp.YTOsiz,cp.YTsiz);
	...
	for(int tileno=0;tileno<cp.th*cp.tw;tileno++)
	{
		...
		tcp->pocs=(POC*)malloc(numPocs* sizeof(POC));
		...
		for(int i=0;i<img.numComponents;i++)
		{
			...
			tccp->codeBlockWidth=jpegMath::int_floorlog2(codeBlockWidth);
			tccp->codeBlockHeight=jpegMath::int_floorlog2(codeBlockHeight);
	...
	IOStream::init(outbuf,cp.XTsiz*cp.YTsiz*cp.tw*cp.th*10);

	jp2Writer writer=new jp2Writer();
	int len=writer->encode(jp2Struct,&cp,outbuf);
...
int jp2_init_stdjp2(jp2Struct * jp2_struct, jp2Image * img)
{
	...
	jp2_struct->h = img->Ysiz - img->YOsiz;	// HEIGHT
	jp2_struct->w = img->Xsiz - img->XOsiz;	// WIDTH
