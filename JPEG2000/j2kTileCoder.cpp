...
j2kTileCoder::j2kTileCoder(CodeParam *comp,jp2Image *image,int currentTile)
{
	...
	widthData=NULL;
	lastSizeOfB=0;
	this->image=new j2kTileCoder_Image();
}
j2kTileCoder::~j2kTileCoder()
{
	delete this->image;
	delete widthData;
}
...
void j2kTileCoder::tcdMallocEncode()
{
	...
	for(int tileno=0;tileno<1;tileno++)
	{
		...
		tile->x0=int_max(cp->XTOsiz+col*cp->XTsiz,img->XOsiz);//如果图像的X起码点>当前tile的X坐标,则tile的X定为图像的起始点
		tile->y0=int_max(cp->YTOsiz+row*cp->YTsiz,img->YOsiz);
		tile->x1=int_min(cp->XTOsiz+(col+1)*cp->XTsiz,img->Xsiz);//如果网格的结束X<图像域的X,则返回网格的X
		tile->y1=int_min(cp->YTOsiz+(row+1)*cp->YTsiz,img->Ysiz);
		...
		for(int compno=0;compno<tile->numComponents;compno++)
		{
			...
			for(int resno=0;resno<tilec->numResolutions;resno++)
			{
				...
				resolution->x0=int_ceildivpow2(tilec->x0,levelno);
				resolution->y0=int_ceildivpow2(tilec->y0,levelno);
				resolution->x1=int_ceildivpow2(tilec->x1,levelno);
				resolution->y1=int_ceildivpow2(tilec->y1,levelno);
				...
				for(int bandno=0;bandno<resolution->numBands;bandno++)
				{
					...
					for(int precno=0;precno<resolution->precinctWidth*resolution->precinctHeight;precno++)
					{
						...
						precinct->inclusionTree=j2kTag::createTagTree(precinct->codeBlockNumInWidth,precinct->codeBlockNumInHeight);
						precinct->imsbTree=j2kTag::createTagTree(precinct->codeBlockNumInWidth,precinct->codeBlockNumInHeight);
		...
		//	tilec->resolutions=(j2kTileCoder_Resolution*)malloc(tilec->numResolutions*sizeof(j2kTileCoder_Resolution));
			...
			//		band->precincts=(j2kTileCoder_Precinct*)malloc(3*resolution->precinctWidth*resolution->precinctHeight*sizeof(j2kTileCoder_Precinct));
					...
					for(int precno=0;precno<resolution->precinctWidth*resolution->precinctHeight;precno++)
					{
						...
						free(precinct->codeBlockInfo);
						precinct->codeBlockInfo=(j2kTileCoder_CodeBlock*)malloc(precinct->codeBlockNumInWidth*precinct->codeBlockNumInHeight*sizeof(j2kTileCoder_CodeBlock));

						if(precinct->inclusionTree!=NULL)
							tag->destory(precinct->inclusionTree);
						if(precinct->imsbTree!=NULL)
							tag->destory(precinct->imsbTree);
...
{
	TileCodeParam *_tcp=&cp->tcps[0];
	TileCompCodeParam *tccp=&_tcp->tccps[0];
...
void j2kTileCoder::dwtEncodeReal(j2kTileCoder_Component *tilec)
{
	...
	for(int i=0;i<numResolution;i++)
	{
		...
		{
			dwtEncodeLowReal(data+j*width,resolutionWidth,1,resolutionLowWidth,ivRow);
		}
	}
}
void j2kTileCoder::dwtEncode(j2kTileCoder_Component *tilec)
{
	...
	for(int i=0;i<numResolution;i++)
	{
		...
		for(int j=0;j<resolutionHeight;j++)
			dwtEncodeLow(data+j*width,resolutionWidth,1,resolutionLowWidth,ivRow);
...
void j2kTileCoder::dwtLazyTransform(int *data,int resWidth,int numRes,int resLowWidth,int iv)
{
	...
	{
		for(int i=0;i<sn;i++)
			widthData[i]=data[(2*i+1)*numRes];
		
		for(int i=0;i<dn;i++)
			widthData[sn+i]=data[2*i*numRes];
	}else{
		for(int i=0;i<sn;i++)
			widthData[i]=data[2*i*numRes];
...
void j2kTileCoder::setCurrentTile(int number)
{
	this->currentTileNo=number;
}